...
import { Department, AddDepartment } from "../../interfaces/department";
import { departmentApi } from "../../api/departmentApi";
import { roomApi } from "../../api/roomApi";
import { Room } from "../../interfaces/room";
import { toast } from "react-toastify";
import { Download, Plus, Component as EntityIcon, X, Loader2 } from "lucide-react";
...
export default function Departments() {
  ...
  const [form, setForm] = useState<AddDepartment>({ department_name: "", room_id: "" });
  ...
  return (
    <div className="bg-[#fcfdff] min-h-screen pb-10">
      <PageMeta title="Departments Management | MOGE" description="Departments Management Page" />

      <div className="px-4 sm:px-8 py-8 max-w-[1400px] mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-xl font-bold text-[#0f172a] tracking-tight">Departments</h3>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div className="bg-white w-80 rounded-xl p-6 border border-gray-200 shadow-sm flex items-center gap-5">
            <div className="p-3.5 bg-[#FEF3C7] rounded-full text-[#B88E2F]"><EntityIcon size={22} /></div>
            <div>
              <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">Total Departments</p>
        ...
        <div>
          <DepartmentTable departments={departments} setDepartments={setDepartments} rooms={rooms} />
        </div>
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col animate-in zoom-in-95 duration-200">
              <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
                <h2 className="text-[17px] font-extrabold text-gray-900">Add New Department</h2>
                <button
              ...
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-[13px] font-bold text-gray-700 mb-2">Department Name <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[13px] font-bold text-gray-700 mb-2">Room <span className="text-red-500">*</span></label>
                  <Select
                    options={rooms.map(r => ({ value: String(r.id), label: `${r.room_name} (No: ${r.room_no})` }))}
                    value={
                      form.room_id
                        ? { value: String(form.room_id), label: rooms.find(r => String(r.id) === String(form.room_id))?.room_name || "Unknown" }
                        : null
                    }
                    onChange={(selected: any) => setForm({ ...form, room_id: selected ? selected.value : "" })}
              ...
              <div className="px-6 py-5 border-t border-gray-100 flex gap-3 justify-end bg-gray-50/50 rounded-b-2xl">
                ...
                <button
                  onClick={async () => {
                    if (!form.department_name.trim() || !form.room_id) {
                    ...
                    try {
                      ...
                      setForm({ department_name: "", room_id: "" });
