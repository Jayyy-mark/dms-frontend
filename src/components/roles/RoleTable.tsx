...
export default function RoleTable({
    ...
    const confirmDelete = async () => {
        ...
        try {
            const msg = await roleApi.delete(deleteTarget.id);
            toast.success(msg || "Deleted.");
            setRoles((prev) => prev.filter((i) => i.id !== deleteTarget.id));
    ...
    const confirmBulkDelete = async () => {
        try {
            await Promise.all([...selectedIds].map(id => roleApi.delete(id)));
            toast.success(`${selectedIds.size} item(s) deleted.`);
            setRoles(prev => prev.filter(i => !selectedIds.has(i.id)));
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            await roleApi.update({
                id: detailItem.id,
                role_id: detailItem.role_id,
                role_name: editName
            });
            toast.success("Role updated successfully!");
            setRoles(prev => prev.map(i => i.id === detailItem.id ? { ...i, role_name: editName } : i));
            setDetailItem(prev => prev ? { ...prev, role_name: editName } : null);
    ...
    const filteredItems = roles.filter(item => {
        if (searchQuery) {
            ...
            const match = item.role_name?.toLowerCase().includes(query);
    ...
    return (
        <div className="w-full">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {currentItems.map((item) => (
                                    <tr key={item.id} onClick={() => setDetailItem(item)} className={`hover:bg-[#FEF3C7]/30 transition-colors group cursor-pointer ${selectedIds.has(item.id) ? 'bg-[#FEF3C7]/50' : ''}`}>
                                        ...
                                        <td className="px-6 py-4 min-w-[200px]"><div className="flex items-center gap-3"><div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold ${getAvatarColor(item.role_name || "R")}`}>{item.role_name?.[0]?.toUpperCase() || "R"}</div><div className="flex flex-col"><span className="text-[14px] font-bold text-[#0f172a] truncate max-w-[180px]">{item.role_name}</span></div></div></td>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {currentItems.map((item) => (
                        <div key={item.id} onClick={() => setDetailItem(item)} className="bg-white rounded-[1.25rem] border border-gray-100 shadow-[0_4px_20px_-8px_rgba(0,0,0,0.08)] p-6 hover:shadow-[0_8px_30px_-8px_rgba(0,0,0,0.12)] transition-all cursor-pointer hover:border-blue-100">
                            <div className="flex justify-between items-start mb-4"><div className="flex items-center gap-4"><div className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${getAvatarColor(item.role_name || "R")}`}>{item.role_name?.[0]?.toUpperCase() || "R"}</div><div className="flex flex-col"><span className="text-[15px] font-extrabold text-[#0f172a] truncate max-w-[180px]">{item.role_name}</span></div></div></div>
                ...
                return (
                    <>
                        ...
                        <div className="fixed z-50 w-44 bg-white rounded-xl shadow-[0_8px_24px_-4px_rgba(0,0,0,0.15)] border border-gray-100 py-1.5" style={{ top: dropdownPos.top, right: dropdownPos.right }}>
                            <button onClick={(e) => { e.stopPropagation(); navigate(`/roles/edit/${item.id}`); }} className="w-full text-left px-4 py-2.5 text-[13px] font-semibold text-gray-700 hover:bg-gray-50 flex items-center gap-3 transition-colors group/item">
            ...
            {detailItem && (() => {
                const avatarClass = getAvatarColor(detailItem.role_name || "R");
                return (
                    <>
                        ...
                        <div className="fixed right-0 top-0 h-full z-50 w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-250">
                            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
                                <span className="text-[15px] font-extrabold text-gray-900">{isEditingDetail ? 'Edit Role' : 'Role Details'}</span>
                            ...
                            <div className="flex-1 overflow-y-auto px-6 py-6" style={{ scrollbarWidth: 'thin', scrollbarColor: '#e2e8f0 transparent' }}>
                                <div className="flex items-center gap-4 mb-6">
                                    <div className={`w-14 h-14 rounded-full flex items-center justify-center text-lg font-extrabold shrink-0 ${avatarClass}`}>
                                        {detailItem.role_name?.[0]?.toUpperCase() || "R"}
                                    </div>
                                    <div className="flex-1">
                                        {isEditingDetail ? (
                                            ...
                                            <h2 className="text-[17px] font-extrabold text-gray-900 leading-tight">{detailItem.role_name}</h2>
                                        )}
                                        <p className="text-[13px] text-gray-400 font-medium mt-1">ID: {detailItem.role_id || `#${detailItem.id}`}</p>
                                    </div>
                                </div>
                            </div>
                            <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
                                {isEditingDetail ? (
                                    ...
                                    <>
                                        <button onClick={() => { setIsEditingDetail(true); setEditName(detailItem.role_name); }} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-gray-200 text-[13px] font-bold text-gray-700 hover:bg-gray-50 transition-colors"><Pencil size={15} /> Edit</button>
