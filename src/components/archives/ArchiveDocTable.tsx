...
import SearchArchiveDocument from "./SearchArchiveDoc";
import {
    MoreHorizontal,
    Trash2,
    Eye,
...
export default function ArchiveDocumentTable({
    documents,
    setDocuments,
    searchRef,
    onSearch,
    ...
    const confirmDelete = async () => {
        ...
        try {
            const data = await documentApi.deleteArchiveDocument(deleteTarget.id);
    ...
    const confirmBulkDelete = async () => {
        try {
            await Promise.all([...selectedIds].map(id => documentApi.deleteArchiveDocument(id)));
    ...
    const handleRestoreDocument = async (document: Document) => {
        try {
            const data = await documentApi.restoreArchiveDocument(document.id);
            toast.success(data?.message);
    ...
    return (
        <div className="w-full">
            ...
            {searchRef && (
                ...
                >
                    <SearchArchiveDocument ref={searchRef} onSearch={handleSearchSubmit} />
                </FilterModal>
            )}
