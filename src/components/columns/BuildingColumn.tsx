...
const ActionCell = ({ building, onEdit, onDelete, rowIndex, totalRows }: { building: Building, onEdit: (b: Building) => void, onDelete: (b: Building) => void, rowIndex: number, totalRows: number }) => {
        ...
        <div className="relative" ref={menuRef}>
            ...
            {isOpen && (
                <div className={`absolute right-0 w-32 bg-white rounded-xl shadow-lg border border-gray-100 z-50 py-1.5 ${isBottom ? 'bottom-full mb-1' : 'mt-1'}`}>
                    <button
                        onClick={() => { onEdit(building); setIsOpen(false); }}
                    ...
                    <button
                        onClick={() => { onDelete(building); setIsOpen(false); }}
...
export const buildingColumns = (
    ...
    onDelete: (building: Building) => void
        ...
        },
        {
            ...
            header: "Building",
            cell: ({ row }) => {
                const nameInitial = row.original.building_name ? row.original.building_name.charAt(0).toUpperCase() : 'B';
                const avatarClass = getAvatarColor(row.original.building_name || 'B');
                return (
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-[12px] font-medium text-gray-400">ID: #{row.original.building_id || row.original.id}</span>
                        </div>
                    </div>
                );
            }
