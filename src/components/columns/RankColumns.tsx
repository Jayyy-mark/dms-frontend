...
const ActionCell = ({ rank, onEdit, onDelete, rowIndex, totalRows }: { rank: Rank, onEdit: (r: Rank) => void, onDelete: (r: Rank) => void, rowIndex: number, totalRows: number }) => {
        ...
        <div className="relative" ref={menuRef}>
            ...
            {isOpen && (
                <div className={`absolute right-0 w-32 bg-white rounded-xl shadow-lg border border-gray-100 z-50 py-1.5 ${isBottom ? 'bottom-full mb-1' : 'mt-1'}`}>
                    <button
                        onClick={() => { onEdit(rank); setIsOpen(false); }}
                    ...
                    <button
                        onClick={() => { onDelete(rank); setIsOpen(false); }}
...
export const rankColumns = (
    ...
    onDelete: (rank: Rank) => void
        ...
        },
        {
            ...
            header: "Rank",
            cell: ({ row }) => {
                const nameInitial = row.original.rank_name ? row.original.rank_name.charAt(0).toUpperCase() : 'R';
                const avatarClass = getAvatarColor(row.original.rank_name || 'R');
                return (
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <span className="text-[14px] font-bold text-[#0f172a] flex items-center gap-1.5">
                                ...
                                {row.original.rank_name}
                            </span>
                            <span className="text-[12px] font-medium text-gray-400">ID: #{row.original.rank_id || row.original.id}</span>
                        </div>
                    </div>
                );
            }
