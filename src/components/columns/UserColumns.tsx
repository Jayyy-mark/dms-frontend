...
const ActionCell = ({ user, onEdit, onDelete, rowIndex, totalRows }: { user: User, onEdit: (u: User) => void, onDelete: (u: User) => void, rowIndex: number, totalRows: number }) => {
...
export const userColumns = (
    ...
    onDelete: (user: User) => void
        ...
        },
        {
            ...
            header: "User",
            cell: ({ row }) => {
                const nameInitial = row.original.username ? row.original.username.charAt(0).toUpperCase() : 'U';
                const avatarClass = getAvatarColor(row.original.username || 'U');
                return (
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <span className="text-[14px] font-bold text-[#0f172a]">{row.original.username}</span>
                            <span className="text-[12px] font-medium text-gray-400">ID: #{row.original.user_id || row.original.id}</span>
                        </div>
                    </div>
                );
            }
        },
        {
            ...
            header: "Email",
            cell: ({ row }) => (
                <span className="text-[13px] font-medium text-gray-600">{row.original.email}</span>
            )
        },
        {
            accessorKey: "role",
