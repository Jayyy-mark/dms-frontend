...
export default function RoomTable({
    ...
    const handleDeleteClick = (item: Room) => {
    ...
    const confirmDelete = async () => {
        ...
        try {
            const msg = await roomApi.delete(deleteTarget.id);
            toast.success(msg || "Deleted.");
            setRooms((prev) => prev.filter((i) => i.id !== deleteTarget.id));
    ...
    const confirmBulkDelete = async () => {
        try {
            await Promise.all([...selectedIds].map(id => roomApi.delete(id)));
            toast.success(`${selectedIds.size} item(s) deleted.`);
            setRooms(prev => prev.filter(i => !selectedIds.has(i.id)));
    ...
    const handleSaveEdit = async () => {
        ...
        if (!editName.trim() || !editNo || !editBuildingId) {
        ...
        try {
            await roomApi.update({
                id: detailItem.id,
                room_id: detailItem.room_id,
                room_name: editName,
                room_no: editNo,
                building_id: editBuildingId
            });
            toast.success("Room updated successfully!");
            const updatedBuilding = buildings.find(b => String(b.id) === editBuildingId);
            setRooms(prev => prev.map(i => i.id === detailItem.id ? {
                ...i,
                room_name: editName,
                room_no: editNo,
                building_id: editBuildingId,
    ...
    const filteredItems = rooms.filter(item => {
        if (searchQuery) {
            ...
            const match = item.room_name?.toLowerCase().includes(query) || item.room_no?.toLowerCase().includes(query);
        ...
        <div className="w-full">
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-2 mb-6">
                <div className="flex items-center mx-2 my-2 px-4 py-1 border border-gray-200 rounded-full bg-white">
                    ...
                    <input type="text" placeholder="Search rooms..." value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }} className="w-full bg-transparent border-none outline-none focus:ring-0 text-sm text-gray-500 px-3 py-2 placeholder-gray-400" />
                </div>
                        ...
                        <table className="w-full min-w-[750px] text-left border-collapse">
                            <thead>
                                <tr className="border-b border-gray-100 bg-gray-50/50">
                                    ...
                                    <th className="px-6 py-4 text-[11px] font-extrabold text-gray-400 uppercase tracking-widest">Room Name / No</th>
                                    <th className="px-6 py-4 text-[11px] font-extrabold text-gray-400 uppercase tracking-widest">Building</th>
                                    ...
                                    <tr key={item.id} onClick={() => setDetailItem(item)} className={`hover:bg-[#FEF3C7]/30 transition-colors group cursor-pointer ${selectedIds.has(item.id) ? 'bg-[#FEF3C7]/50' : ''}`}>
                                        ...
                                        <td className="px-6 py-4 min-w-[200px]">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold ${getAvatarColor(item.room_name || "R")}`}>{item.room_name?.[0]?.toUpperCase() || "R"}</div>
                                                <div className="flex flex-col">
                                                    <span className="text-[14px] font-bold text-[#0f172a] truncate max-w-[180px]">{item.room_name}</span>
                                                    <span className="text-[12px] font-medium text-gray-400 truncate max-w-[180px]">No: {item.room_no}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="text-[13px] font-medium text-gray-600 truncate max-w-[180px]">{item.building?.building_name || "—"}</div>
                                        </td>
                        ...
                        <div key={item.id} onClick={() => setDetailItem(item)} className="bg-white rounded-[1.25rem] border border-gray-100 shadow-[0_4px_20px_-8px_rgba(0,0,0,0.08)] p-6 hover:shadow-[0_8px_30px_-8px_rgba(0,0,0,0.12)] transition-all cursor-pointer hover:border-blue-100">
                            <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-4">
                                    <div className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${getAvatarColor(item.room_name || "R")}`}>{item.room_name?.[0]?.toUpperCase() || "R"}</div>
                                    <div className="flex flex-col">
                                        <span className="text-[15px] font-extrabold text-[#0f172a] truncate max-w-[180px]">{item.room_name}</span>
                                        <span className="text-[13px] font-medium text-gray-500 truncate max-w-[180px]">No: {item.room_no}</span>
                                        <span className="text-[12px] font-medium text-gray-400 mt-1">Building: {item.building?.building_name || "—"}</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <>
                        ...
                        <div className="fixed z-50 w-44 bg-white rounded-xl shadow-[0_8px_24px_-4px_rgba(0,0,0,0.15)] border border-gray-100 py-1.5" style={{ top: dropdownPos.top, right: dropdownPos.right }}>
                            <button onClick={(e) => {
                                ...
                                setEditName(item.room_name);
                                setEditNo(item.room_no);
                                setEditBuildingId(String(item.building_id));
            ...
            {detailItem && (() => {
                const avatarClass = getAvatarColor(detailItem.room_name || "R");
                return (
                    <>
                        ...
                        <div className="fixed right-0 top-0 h-full z-50 w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-250">
                            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
                                <span className="text-[15px] font-extrabold text-gray-900">{isEditingDetail ? 'Edit Room' : 'Room Details'}</span>
                            ...
                            <div className="flex-1 overflow-y-auto px-6 py-6" style={{ scrollbarWidth: 'thin', scrollbarColor: '#e2e8f0 transparent' }}>
                                <div className="flex items-start gap-4 mb-6">
                                    <div className={`w-14 h-14 rounded-full flex items-center justify-center text-lg font-extrabold shrink-0 ${avatarClass}`}>
                                        {detailItem.room_name?.[0]?.toUpperCase() || "R"}
                                    </div>
                                    <div className="flex-1 space-y-4">
                                        {isEditingDetail ? (
                                            <>
                                                <div>
                                                    <label className="block text-[11px] font-extrabold text-gray-400 uppercase tracking-widest mb-1.5">Room Name</label>
                                                    <input
                                                        autoFocus
                                                        type="text"
                                                        value={editName}
                                                ...
                                                <div>
                                                    <label className="block text-[11px] font-extrabold text-gray-400 uppercase tracking-widest mb-1.5">Room No.</label>
                                                    <input
                                                        type="text"
                                                        value={editNo}
                                                ...
                                                <div>
                                                    <label className="block text-[11px] font-extrabold text-gray-400 uppercase tracking-widest mb-1.5">Building</label>
                                                    <Select
                                                        options={buildings.map(b => ({ value: String(b.id), label: b.building_name }))}
                                                        value={
                                                            editBuildingId
                                                                ? { value: String(editBuildingId), label: buildings.find(b => String(b.id) === String(editBuildingId))?.building_name || "Unknown" }
                                                                : null
                                                        }
                                                        onChange={(selected: any) => setEditBuildingId(selected ? selected.value : "")}
                                            ...
                                            <>
                                                <div>
                                                    <h2 className="text-[17px] font-extrabold text-gray-900 leading-tight">{detailItem.room_name}</h2>
                                                    <p className="text-[13px] text-gray-500 font-medium mt-1">No: {detailItem.room_no}</p>
                                                    <p className="text-[12px] text-gray-400 font-medium mt-0.5">ID: {detailItem.room_id || `#${detailItem.id}`}</p>
                                                </div>
                                                <div className="pt-4 border-t border-gray-100">
                                                    <p className="text-[11px] font-extrabold text-gray-400 uppercase tracking-widest mb-1.5">Building</p>
                                                    <p className="text-[14px] font-bold text-gray-800">{detailItem.building?.building_name || "—"}</p>
                                                </div>
                                            </>
                                        )}
                                    </div>
                            ...
                            <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
                                {isEditingDetail ? (
                                    ...
                                    <>
                                        <button onClick={() => {
                                            setIsEditingDetail(true);
                                            setEditName(detailItem.room_name);
                                            setEditNo(detailItem.room_no);
                                            setEditBuildingId(String(detailItem.building_id));
