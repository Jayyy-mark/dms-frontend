...
import DocumentFilterDrawer, { FilterState, initialFilterState } from "./DocumentFilterDrawer";
import {
    MoreHorizontal,
    Pencil,
    Trash2,
...
export default function DocumentTable({
    ...
    const swr = useDocuments();
    const [localDocuments, setLocalDocuments] = useState<Document[]>([]);

    useEffect(() => {
        if (propDocuments !== undefined) {
            setLocalDocuments(propDocuments);
        } else if (swr.documents) {
            setLocalDocuments(swr.documents);
        }
    }, [propDocuments, swr.documents]);

    const documents = propDocuments !== undefined ? propDocuments : localDocuments;
    const setDocuments = propSetDocuments || setLocalDocuments;
    const mutate = propMutate || swr.mutate;
    ...
    // Drawer Filter state (persistent until cleared)
    const [drawerFilterState, setDrawerFilterState] = useState<FilterState>(initialFilterState);
    const [isFilterDrawerOpen, setIsFilterDrawerOpen] = useState(false);

    const hasActiveDrawerFilters =
        drawerFilterState.sortBy !== "recent" ||
        drawerFilterState.docTypes.length > 0 ||
        drawerFilterState.statuses.length > 0 ||
        drawerFilterState.staffIds.length > 0 ||
        drawerFilterState.categoryIds.length > 0;
    ...
    const toggleDropdown = (id: number, e: React.MouseEvent<HTMLButtonElement>) => {
        if (openDropdownId === id) {
            ...
            // Estimate dropdown height to be ~180px
    ...
    // Filter & Sort Logic (Evaluating persistent Drawer Filter State)
    let filteredDocs = documents.filter(doc => {
        // Tab filter
        const status = getDocStatus(doc);
        if (activeTab !== 'all' && status !== activeTab) return false;

        // Category filter (combines top category modal and drawer category tree)
        const activeCatIds = drawerFilterState.categoryIds.length > 0 ? drawerFilterState.categoryIds : selectedCategories;
        if (activeCatIds.length > 0) {
            if (!doc.category?.id || !activeCatIds.includes(doc.category.id)) {
                return false;
            }
        }

        // Document Type filter (permanent / temporary)
        if (drawerFilterState.docTypes.length > 0) {
            if (!drawerFilterState.docTypes.includes(status)) return false;
        }

        // Status filter (active / expired)
        if (drawerFilterState.statuses.length > 0) {
            if (drawerFilterState.statuses.includes("active") && status === "expired") return false;
            if (drawerFilterState.statuses.includes("expired") && status !== "expired") return false;
        }

        // Staff Member filter
        if (drawerFilterState.staffIds.length > 0) {
            const staffIdStr = String(doc.staff?.id || doc.staff_id || "");
            if (!drawerFilterState.staffIds.includes(staffIdStr)) return false;
        }
    ...
    // Apply Sorting
    if (drawerFilterState.sortBy === "older") {
        filteredDocs = [...filteredDocs].sort((a, b) => new Date(a.created_at || 0).getTime() - new Date(b.created_at || 0).getTime());
    } else if (drawerFilterState.sortBy === "alphabetical") {
        filteredDocs = [...filteredDocs].sort((a, b) => (a.document_name || "").localeCompare(b.document_name || ""));
    } else if (drawerFilterState.sortBy === "recent") {
        filteredDocs = [...filteredDocs].sort((a, b) => new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime());
    }
    ...
    const getFileExt = (doc: Document) => {
        const path = (doc.document as any);
        if (!path || typeof path !== 'string') return '';
        return path.split('.').pop()?.toLowerCase() || '';
    };

    const getFileIcon = (doc: Document, size: number = 16) => {
        const ext = getFileExt(doc);
        switch (ext) {
            case 'pdf':
                return { icon: <FileText size={size} />, bg: 'bg-red-100', text: 'text-red-600' };
            case 'doc':
            case 'docx':
                return { icon: <FileText size={size} />, bg: 'bg-blue-100', text: 'text-blue-600' };
            case 'xls':
            case 'xlsx':
            case 'csv':
                return { icon: <FileSpreadsheet size={size} />, bg: 'bg-emerald-100', text: 'text-emerald-600' };
            case 'ppt':
            case 'pptx':
                return { icon: <Presentation size={size} />, bg: 'bg-orange-100', text: 'text-orange-600' };
            case 'png':
            case 'jpg':
            case 'jpeg':
            case 'gif':
            case 'webp':
                return { icon: <FileImage size={size} />, bg: 'bg-pink-100', text: 'text-pink-600' };
            case 'zip':
            case 'rar':
            case '7z':
                return { icon: <FileArchive size={size} />, bg: 'bg-yellow-100', text: 'text-yellow-600' };
            default:
                return { icon: <File size={size} />, bg: 'bg-gray-100', text: 'text-gray-500' };
        }
    };
        ...
        <div className="w-full">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className={`flex items-center gap-1.5 border rounded-full px-4 py-1.5 text-[13px] font-semibold outline-none cursor-pointer transition-all ${selectedCategories.length > 0
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold transition-all shadow-sm ${hasActiveDrawerFilters
                                ? 'bg-[#997524] text-white border border-[#80611d]'
                                : 'bg-[#B88E2F] hover:bg-[#997524] text-white'
                                }`}
                        >
            ...
            {viewMode === "list" ? (
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 min-w-[250px]">
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-xs font-bold ${fileIcon.bg} ${fileIcon.text}`}>
                                                        {fileIcon.icon}
                                                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {currentDocuments.map((doc) => {
                        const status = getDocStatus(doc);
                        const fileIcon = getFileIcon(doc, 20);
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center gap-4">
                                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-sm font-bold shrink-0 ${fileIcon.bg} ${fileIcon.text}`}>
                                            {fileIcon.icon}
                                        </div>
                                        <div className="flex flex-col">
                                            <a
                                                    ...
                                                    <div className="absolute right-0 mt-1 w-40 bg-white rounded-xl shadow-[0_5px_15px_-3px_rgba(0,0,0,0.1)] border border-gray-100 z-20 py-2 animate-in fade-in zoom-in-95 duration-100">
                                                        ...
                                                        {status !== "temporary" && (
                                                            <button
                                                                ...
                                                                className="w-full text-left px-4 py-2 text-[13px] font-semibold text-gray-700 hover:bg-gray-50 hover:text-[#B88E2F] flex items-center transition-colors"
                                                            >
                                                                <ArchiveIcon size={15} className="mr-2" /> Archive
                                                            </button>
                                                        )}
                    ...
                    <>
                        ...
                        >
                            ...
                            <button
                                ...
                                className="w-full text-left px-4 py-2.5 text-[13px] font-semibold text-gray-700 hover:bg-gray-50 flex items-center gap-3 transition-colors group/item"
                            >
                                <div className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center group-hover/item:bg-gray-200 transition-colors">
                            ...
                            {status !== "temporary" && (
                                <button
                                    ...
                                    className="w-full text-left px-4 py-2.5 text-[13px] font-semibold text-gray-700 hover:bg-gray-50 flex items-center gap-3 transition-colors group/item"
                                >
                                    <div className="w-7 h-7 rounded-lg bg-amber-50 flex items-center justify-center group-hover/item:bg-amber-100 transition-colors">
                                        <ArchiveIcon size={14} className="text-amber-500" />
                                    </div>
                                    Archive
                                </button>
                            )}
            ...
            {searchRef && (
                ...
                >
                    <SearchDocument ref={searchRef} onSearch={handleSearchSubmit} />
                </FilterModal>
            )}
            ...
            {detailDoc && (() => {
                ...
                const detailFileIcon = getFileIcon(detailDoc, 24);
                const categoryPath = renderCategoryPath(detailDoc.category);
                const fileName = getFileName(detailDoc);
                return (
                    <>
                        ...
                        <div className="fixed right-0 top-0 h-full z-50 w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-250">
                            ...
                            <div className="flex-1 overflow-y-auto px-6 py-6" style={{ scrollbarWidth: 'thin', scrollbarColor: '#e2e8f0 transparent' }}>
                                ...
                                <div className="flex items-center gap-4 mb-6">
                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-lg font-extrabold shrink-0 ${detailFileIcon.bg} ${detailFileIcon.text}`}>
                                        {detailFileIcon.icon}
                                    </div>
                                    <div>
                                ...
                                {fileName && (
                                    <div className="mb-6 p-3 rounded-xl bg-gray-50 border border-gray-100">
                                        <p className="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-1">Filename</p>
                                        <p className="text-[13px] font-semibold text-gray-700 break-all">{fileName}</p>
                                    </div>
                                )}

                                {/* Status + category badges */}
                                <div className="flex flex-wrap gap-2 mb-6">
                                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-extrabold tracking-wide ${status === 'permanent' ? 'bg-[#f0fdf4] text-[#22c55e]' :
                                        status === 'temporary' ? 'bg-[#eff6ff] text-[#3b82f6]' :
                                            'bg-[#fef2f2] text-[#ef4444]'
                                        }`}>
                            ...
                            <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
                                ...
                                <button
                                    onClick={() => { handleEditDocument(detailDoc); setDetailDoc(null); }}
                                    className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-gray-200 text-[13px] font-bold text-gray-700 hover:bg-gray-50 transition-colors"
                                >
                                    <Pencil size={15} /> Edit
                                </button>
            ...
            {/* Document Filter Right Offcanvas Drawer */}
            <DocumentFilterDrawer
                isOpen={isFilterDrawerOpen}
                onClose={() => setIsFilterDrawerOpen(false)}
                filterState={drawerFilterState}
                onApplyFilter={(newState) => {
                    setDrawerFilterState(newState);
                    setCurrentPage(1);
                }}
                onClearFilter={() => {
                    setDrawerFilterState(initialFilterState);
                    setCurrentPage(1);
                }}
            />
        </div>
